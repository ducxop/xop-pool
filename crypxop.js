...
var getData = ()=>{
    ...
    var handleCM = res=>{
        data[0][0] = res.data['1'].quotes.USD.price
        data[1][0] = res.data['1831'].quotes.USD.price
        data[2][0] = res.data['1027'].quotes.USD.price
        data[3][0] = Number(res.data['52'].quotes.USD.price.toFixed(5))
        data[4][0] = res.data['1765'].quotes.USD.price
        complete()
    }
    binance.prices((error, ticker) => {
        if (!error){
            ...
            data[3][1] = Number(Number(ticker.XRPUSDT).toFixed(5))
            data[4][1] = Number(ticker.EOSUSDT)
        }
        else{            
            data[0][1] = 0
            data[1][1] = 0
    ...
    var handleBS = res=>{
        ...
        data[1][2] = Number(res[1].last)
        data[2][2] = Number(res[2].last)
        data[3][2] = Number(Number(res[3].last).toFixed(5))
...
getData()
var objItv = setInterval(getData, 10000)
